...
<template>
  ...
  <div class="bg-gray-200 h-screen flex items-center">
    <div
      class="bg-white dark:bg-surface-900 p-8 md:p-12 shadow-sm rounded-2xl w-full max-w-md mx-auto flex flex-col gap-8">
      ...
      <div class="flex flex-col gap-6 w-full">
        <Form v-slot="$form" ref="formData" :resolver="resolver" @submit="onFormSubmit" class="flex flex-col gap-6">
          ...
          <FormField v-slot="$field" name="password" class="flex flex-col gap-1">
            ...
            <InputText id="password" type="password" v-model="$field.value" placeholder="password"
              :class="{ 'p-invalid': $field.invalid }" />
            <Message v-if="$field?.invalid" severity="error" size="small" variant="simple">
              {{ $field.error?.message }}
            </Message>
